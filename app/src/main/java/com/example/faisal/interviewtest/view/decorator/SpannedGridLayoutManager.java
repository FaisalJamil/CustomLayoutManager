...
package com.example.faisal.interviewtest.view.decorator;
...
public class SpannedGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    private int getXbyXColumns(int itemCount){
        int remaining_columns = 0;
        int remaider = itemCount % this.columns;
        if(remaider != 0){
            remaining_columns = this.columns - remaider;
        }
        return itemCount + remaining_columns;
    }
    /**
    ...
    private void calculateCellPositions(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // we need to get an X by X grid
        int cellsCount = getXbyXColumns(itemCount);
        cells = new SparseArray<>(cellsCount);
        ...
        for (int position = 0; position < itemCount; position++) {
            ...
            if (column + spanInfo.columnSpan > columns) {
                ...
                if(row % 2 == 0) //even rows start from right
                    column = columns -1;
                else
                    column = 0; //odd rows start from left
            }
            ...
            if(row != 0 && row % 2 == 0 && column != 0) // if row is even, not the first row, and not the last column
                    column -= spanInfo.columnSpan;
            else if(row % 2 != 0)//if row is odd
                column += spanInfo.columnSpan; // increment the current position
